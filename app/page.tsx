...
export default function Sakumlapa() {
  ...
  // Funkcija, kas pilnībā atiestata sākumlapu (notīra filtrus)
  ...
  return (
    <div style={{ width: '100%', minHeight: '100vh', backgroundColor: '#fff', boxSizing: 'border-box', fontFamily: 'sans-serif' }}>
      
      {/* AUGŠĒJĀ NAVIGĀCIJAS JOSLA AR TEMAUTO LOGO */}
      <header style={{ backgroundColor: '#111827', padding: '12px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 1000 }}>
        <a 
          href="/" 
          onClick={resetToHome} 
          style={{ color: '#22c55e', fontSize: '22px', fontWeight: 'bold', textDecoration: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px' }}
        >
          TemAuto
        </a>
        <div style={{ color: '#9ca3af', fontSize: '13px' }}>
          Apmeklētāji 24h: <span style={{ color: '#22c55e', fontWeight: 'bold' }}>0</span>
        </div>
      </header>

      {/* GALVENĀS LAPAS SATURS */}
      <div style={{ padding: '16px 8px', width: '100%', boxSizing: 'border-box' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '270px 1fr 240px', gap: '16px', alignItems: 'flex-start', width: '100%' }}>
          
          {/* KREISĀ PUSE: Marku saraksts (stingri fiksēts skrollējot) */}
          <div style={{ position: 'sticky', top: '70px', height: 'calc(100vh - 90px)', overflowY: 'auto', backgroundColor: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '8px', padding: '8px', boxSizing: 'border-box' }}>
            {loading ? (
