...
const MODELS_BY_MAKE: { [key: string]: string[] } = {
  ...
  'Toyota': ['Corolla', 'Camry', 'RAV4', 'Land Cruiser', 'Yuris', 'Avensis', 'C-HR'],
...
// Ģenerēsim gadus no pašreizējā gada līdz 1970. gadam
...
export default function PievienotAuto() {
  ...
  // Cenas formatēšana ar tūkstošu atdalītājiem
  ...
  // Modelu saraksts balstīts uz izvēlēto marku
  const availableModels = (MODELS_BY_MAKE[make] || ['Sedans', 'Universāls', 'Kupee']).filter(m => 
    m.toLowerCase().includes(model.toLowerCase())
  )

  return (
    <div ref={dropdownRef} style={{ maxWidth: '800px', margin: '40px auto', padding: '24px', backgroundColor: '#ffffff', border: '1px solid #e5e7eb', borderRadius: '12px', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.05)', boxSizing: 'border-box' }}>
      ...
      <div style={{ marginBottom: '24px', borderBottom: '1px solid #e5e7eb', paddingBottom: '16px' }}>
        ...
        <p style={{ fontSize: '13.5px', color: '#6b7280', marginTop: '4px' }}>Aizpildiet zemāk esošos laukus. Izmantojiet sagatavotos sarakstus un ieteikumus ātrākai datu ievadei.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
          ...
          {/* Gads ar kalendāra/gadu izvēlni */}
          <div style={{ position: 'relative' }}>
        ...
        {/* KRĀSA AR KRĀSAINIEM PARAUDZIŅIEM */}
        <div style={{ position: 'relative' }}>
        ...
        {/* BILDES URL */}
        <div>
          <label style={{ display: 'block', fontSize: '13px', fontWeight: 'bold', color: '#374151', marginBottom: '6px' }}>Attēlu saites (URL)</label>
          {images.map((img, index) => (
            <div key={index} style={{ display: 'flex', gap: '8px', marginBottom: '8px' }}>
              <input
                type="text"
                placeholder="Ielīmējiet attēla saiti (https://...)"
                value={img}
              ...
              {images.length > 1 && (
                <button
                  ...
                  style={{ padding: '0 12px', backgroundColor: '#fee2e2', color: '#991b1b', border: '1px solid #fecaca', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold' }}
                >
                  ✕
                </button>
              )}
          ...
          >
            + Pievienot vēl vienu attēla saiti
          </button>
        </div>
